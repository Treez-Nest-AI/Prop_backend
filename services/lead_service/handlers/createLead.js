  ...
  try {
    ...
    if (!phoneRegex.test(phone)) {
      return {
        statusCode: 400,
        headers: corsHeaders,
        body: JSON.stringify({ message: "Phone must be 10 digits" }),
      };
    }
    ...
    if (visitDate < today) {
      return {
        statusCode: 400,
        headers: corsHeaders,
        body: JSON.stringify({ message: "Past dates are not allowed" }),
      };
    }
